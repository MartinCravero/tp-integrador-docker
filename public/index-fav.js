...
//////SECCION FAVORITOS//////
    ...
    async function getGifFavorites (){
        let gifFavorites = localStorage.getItem("gifFavorites");
        let linkSearchFav = `https://api.giphy.com/v1/gifs?api_key=${apikey}&ids=${gifFavorites}`
        let response = await fetch(linkSearchFav)
    ...
    getGifFavorites().then(response=>{
        ...
        if (response.data.length !== 0) {
            ...
            gifosResults.classList.add('gifFavorites')
...
///FUNCIÓN AGREGAR A FAVORITOS///
        ...
        let noFavContainer = document.createElement("div")
        gifosResults.classList.add('noFavorites')
        noFavContainer.innerHTML=`
            <div class="favNoCont">
                <img class="favNoContImg" src="./iconos/icon-fav-sin-contenido.svg" alt="">
                <h2 class="favNoContText">"¡Guarda tu primer GIFO en Favoritos para que se muestre aquí!"</h2>
            </div>`
        gifosResults.appendChild(noFavContainer)
    }
